...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{./results/figures/fig8_roc_curve.pdf}
...
=======
\subsection{Score Distributions}
Table~\ref{tab:dist} reports the marginal distributions of the VLM
and the three-annotator average. The VLM exhibits a strongly bimodal
pattern: 77--81 of the 100 videos receive a score of exactly 0.0 on
each axis, with a smaller cluster at 0.6 or above. Human annotators
use the intermediate values 0.2 and 0.4 more frequently, producing
higher per-axis means despite broad agreement on which clips are
clean. The VLM labels 29 videos as poisoned and the human reference
(any-dimension mean $\ge\!0.5$) labels 27, within 2 of the 30\%
official prior.
>>>>>>> upstream/main
...
\begin{tabular}{lcccc}
\toprule
Axis & VLM $\mu$ & Human $\mu$ & \#Zero & $\ge\!0.6$ \\
=======
\begin{tabular}{lcccc}
\toprule
...
\end{tabular}
<<<<<<< HEAD
\caption{Marginal score distributions ($n\!=\!100$).
\#Zero: videos scoring exactly 0.0.}
\label{tab:dist}
\end{table}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{./results/figures/fig5_distribution_violin.pdf}
...
\subsection{Binary Poisoned Detection}
Using a human threshold of $0.5$ and a VLM threshold of $0.6$, the
confusion matrix is TP $=21$, FP $=8$, FN $=6$, TN $=65$, yielding
accuracy 0.860, precision 0.724, recall 0.778, and F1 0.750
(Figure~\ref{fig:cm}). Treating $\max(\text{sem},\text{log},\text{dec})$
as a continuous confidence score gives an ROC AUC of 0.839
(Figure~\ref{fig:roc}), which is threshold-independent and therefore
a cleaner summary of the judge's discriminative ability than any
single operating point. The six false negatives are dominated by
Logical-axis cases in which the VLM scored below 0.6 because no single
object was clearly missing in any individual frame, despite high
cumulative drift across the 8-frame sequence.
>>>>>>> upstream/main
...
\begin{tabular}{lcccc}
\toprule
Axis & $\rho$ & Wt.\ $\kappa$ & ICC\textsubscript{H-V} & ICC\textsubscript{H-H} \\
=======
\begin{tabular}{lcccc}
\toprule
...
\midrule
<<<<<<< HEAD
Overall  & \textbf{0.587} & --- & 0.536 & --- \\
\bottomrule
\end{tabular}
\caption{Human--VLM agreement ($n\!=\!100$, all $p\!<\!0.001$).
$\rho$: Spearman. Bold: VLM matches inter-human ceiling.}
=======
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{./results/figures/fig6_per_video_deviation.pdf}
\caption{Per-video deviation (VLM $-$ human average) on the weighted
final score. Negative bars indicate VLM under-scoring.}
\label{fig:deviation}
\end{figure*}
=======
inter-human ceiling of 0.522, indicating that the model's
entity-level reliability is comparable to that of an average human
annotator. On Logical and Decision the Spearman rank correlation
remains comparable to Semantic ($\rho\!\approx\!0.50$), so the
relative ordering of videos is preserved, but the absolute scores
are systematically lower (human 0.23 vs.\ VLM 0.06 on Logical;
human 0.25 vs.\ VLM 0.08 on Decision). The ICC, which is sensitive
to absolute agreement, penalises this offset.

\subsection{Visual Diagnostics}
Figure~\ref{fig:scatter} presents the per-axis human-vs-VLM scatter
with linear fits. The Semantic points cluster along the diagonal,
whereas the Logical and Decision points lie consistently below it.
The radar plot in Figure~\ref{fig:radar} aggregates the four metrics
across axes, making the gap between Semantic and the other two
visible at a glance.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../results/figures/fig1_scatter_regression.png}
\caption{Human-average vs.\ VLM scores per axis with linear fits.}
\label{fig:scatter}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{../results/figures/fig4_radar_metrics.png}
\caption{Agreement metrics on a radar plot.}
\label{fig:radar}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../results/figures/fig3_confusion_matrix.png}
\caption{Confusion matrix for binary poisoned detection
(human $\ge\!0.5$, VLM $\ge\!0.6$).}
\label{fig:cm}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{../results/figures/fig8_roc_curve.png}
\caption{ROC curve on 100 videos, with
$\max(\text{sem},\text{log},\text{dec})$ as the continuous
VLM confidence. AUC $=0.839$.}
\label{fig:roc}
...
=======
The Semantic ICC of 0.510 sits within one standard error of the
inter-human ceiling of 0.522, indicating that the VLM has reached
the noise floor for entity-level scoring; further prompt tuning on
this axis is bounded by human variability rather than model
capability. The zero false-positive rate across all three robustness
controls supports the interpretation that this agreement is driven
by visual grounding rather than by the text-channel statistics that
a prompt-shortcut judge could also exploit. The under-scoring on Logical and Decision is primarily
a prompt artefact: the noise-calibration clause introduced to
suppress the 0.2 floor effect produced systematically conservative
scores. Relaxing this clause for the Decision axis, where the cost
of a missed safety risk exceeds that of a noisy false alarm, is a
natural next ablation. The bimodal score distribution is acceptable
for binary poisoned detection but would distort any downstream
continuous ranking task; a calibration regressor trained on a
held-out human subset could plausibly recover the intermediate
values.
>>>>>>> upstream/main
...
Two JSON files are submitted alongside this report:
\begin{itemize}
  \item \texttt{annotation.json} --- per-video evaluation results
        for all 100 clips, containing both human and LLM assessments.
        Each entry includes \texttt{video\_id},
        \texttt{human\_evaluation} (three-annotator average scores,
        \texttt{is\_poisoned}, \texttt{attack\_level},
        \texttt{final\_score}) and \texttt{llm\_evaluation}
        (VLM scores, \texttt{reasoning}).
  \item \texttt{evaluation\_criteria\&prompt\_design.json} ---
        the full evaluation criteria (three-axis scoring rubric,
        aggregation rules, noise calibration, pixel-deviation anchors)
        and the complete prompt template (v2.7) used for VLM inference,
        including the revision history from v2.0 to v2.7.
\end{itemize}
