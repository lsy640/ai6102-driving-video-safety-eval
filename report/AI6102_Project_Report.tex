...
\{xjiang026, lius0131, lyunhao001\}@e.ntu.edu.sg
}
...
classification the model reaches F1 0.746 and matches the official
30\% prior almost exactly. The per-axis ICC$(2,1)$ is more nuanced:
...
the per-video frame grids in \texttt{frames\_8/} rather than on the
original MP4 files; sequential viewing of the eight PNGs reproduces
the 4 FPS animation while permitting pause on a single frame for
detailed inspection. Scores were recorded in \texttt{annotation.xlsx}
...
clean. The two sets of labels classify 29 and 30 videos as poisoned
respectively, indicating near-identical performance on the binary
task.
...
At the official $\ge\!0.6$ threshold the confusion matrix is
TP $=22$, FP $=7$, FN $=8$, TN $=63$, yielding precision 0.759,
recall 0.733, and F1 0.746 (Figure~\ref{fig:cm}). The eight false
negatives are dominated by Logical-axis cases in which the VLM
scored below 0.6 because no single object was clearly missing in any
individual frame, despite high cumulative drift across the 8-frame
sequence.
...
\caption{Confusion matrix for binary poisoned detection at the
$\ge\!0.6$ threshold.}
\label{fig:cm}
\end{figure}

\subsection{Failure Modes}
Manual inspection of the eight false negatives and a sample of false
...
The pipeline was re-run three times on a 10-video subset under
modified input conditions. Removing the images and supplying only
the annotation summary and drift percentage returned near-zero
scores on every axis, consistent with the prompt's default-to-zero
rule when visual evidence is absent. Replacing every frame with
uniform random noise produced elevated Logical scores and low
Semantic scores, as expected from the absence of recognisable
entities and the lack of temporal coherence. Replacing the bottom
strip with the top strip --- a perfect reconstruction --- yielded
all-zero outputs with no false positives.
...
capability. The under-scoring on Logical and Decision is primarily
...
\section{Conclusion}
The proposed framework attains F1 0.746 on binary poisoned detection
and matches inter-human reliability on the Semantic axis, while
under-scoring on Logical and Decision. The under-scoring is
